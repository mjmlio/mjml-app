...
export default handleActions(
  {
    TAB_OPEN: (state, { payload: filePath }) => {
      ...
      if (foundIndex > -1) {
        return state.map(t => t.set('isFocused', false)).setIn([foundIndex, 'isFocused'], true)
      }
      return state.map(t => t.set('isFocused', false)).push(
        fromJS({
          path: filePath,
      ...
      if (state.getIn([indexToRemove, 'isFocused']) === true) {
        ...
        const indexToFocus =
          state.size > 1
            ? indexToRemove === 0
              ? 1
              : indexToRemove === state.size - 1 ? indexToRemove - 1 : indexToRemove + 1
            : -1
