...
class TabContent extends Component {
  ...
  renderMJML(tab) {
    return (
      <SplitPane className="sticky" split="vertical" primary="second" minSize={300}>
        <CodeEditor filePath={tab.get('path')} onChange={v => this.props.setTabValue(v)} />
        <TabIframe tab={tab} />
      </SplitPane>
    )
  }
  ...
  renderTab(tab) {
    ...
    switch (ext) {
      case '.mjml':
        return this.renderMJML(tab)
      case '.png':
      case '.jpg':
        return this.renderImage(tab)
      default:
        return this.renderUnhandled(ext)
    }
  }
