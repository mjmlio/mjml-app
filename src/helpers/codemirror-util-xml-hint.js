...
;(function(mod) {
  ...
  function getHints(cm, options) {
    ...
    var tagStart
    var trigger = token.type == 'trigger'
    ...
    if (triggerName) {
      var before = cm.getLine(cur.line).slice(Math.max(0, token.start - 2), token.start)
      var triggerType = /\+$/.test(before) ? 'trigger' : null
      tagStart = token.start
    }

    if (((!tag && !inner.state.tagName) || tagType) && options.tags == 'mjml') {
    ...
    } else if (((!tag && !inner.state.tagName) || triggerType) && options.tags == 'snippets') {
      if (triggerName) prefix = token.string
      replaceToken = triggerType
